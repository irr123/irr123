...
security]({{< relref "blog/posts/html-sanitization" >}}) -- here's the
adversarial round.
...
that exploits it. Everything else -- the red banner, the CVSS score, the "we
should really" -- is a personal take. Mostly marketing. Adding security you
can't justify isn't rigor -- it's security theater. And it's a vicious cycle.

## Talk is cheap. Show me the code.[^torvalds]

A working proof of concept is proof. A severity score is a claim. A scanner
label is a claim. Someone's opinion is a claim. Claims are cheap -- generate a
thousand a day, no code required. The PoC is the tax that separates the real
from the loud.
...
None of this is anti-tooling; the enemy isn't the scanner or the model. It's
treating a claim as a verdict -- a UI/UX problem in the tools, and blind
obedience to decade-old compliance checklists.
